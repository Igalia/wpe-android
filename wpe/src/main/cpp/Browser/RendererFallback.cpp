    ...
    , m_size({width, height})
{
    ...
    m_egl.createImageKHR = reinterpret_cast<PFNEGLCREATEIMAGEKHRPROC>(eglGetProcAddress("eglCreateImageKHR"));
    m_egl.destroyImageKHR = reinterpret_cast<PFNEGLDESTROYIMAGEKHRPROC>(eglGetProcAddress("eglDestroyImageKHR"));
    m_egl.imageTargetTexture2DOES
...
static const char* s_vertexShaderSource = "attribute vec2 pos;\n"
                                          ...
                                          "}\n";
static const char* s_fragmentShaderSource = "precision mediump float;\n"
...
void RendererFallback::surfaceCreated(ANativeWindow* window)
{
    ...
    static const EGLint configAttributes[] = {EGL_RENDERABLE_TYPE, EGL_OPENGL_ES2_BIT, EGL_RED_SIZE, 1, EGL_GREEN_SIZE,
        1, EGL_BLUE_SIZE, 1, EGL_ALPHA_SIZE, 1, EGL_SURFACE_TYPE, EGL_WINDOW_BIT, EGL_NONE};
    ...
    ALOGV("RendererFallback: shaders %d/%d program %d texture %u", m_gl.vertexShader, m_gl.fragmentShader, m_gl.program,
        m_gl.texture);
}
...
void RendererFallback::scheduleFrame(FrameContext* frameContext)
{
    ...
    ALooper_addFd(looper, m_update.fd, ALOOPER_POLL_CALLBACK, ALOOPER_EVENT_INPUT, s_updateCallback, frameContext);
...
void RendererFallback::renderFrame(const std::shared_ptr<ExportedBuffer>& buffer, bool dispatchFrameCompleteCallback)
{
...

    {
        if (m_lockedBuffer) {
            wpe_android_view_backend_exportable_dispatch_release_buffer(
                m_page.exportable(), m_lockedBuffer->buffer, m_lockedBuffer->poolID, m_lockedBuffer->bufferID);
    ...
    eglMakeCurrent(m_egl.display, m_egl.surface, m_egl.surface, m_egl.context);
    {
        ...
        EGLImageKHR image
            = m_egl.createImageKHR(m_egl.display, EGL_NO_CONTEXT, EGL_NATIVE_BUFFER_ANDROID, clientBuffer, nullptr);
        ALOGV("RendererFallback: clientBuffer %p => image %p, err %x, size (%ux%u)", clientBuffer, image, eglGetError(),
...
#endif
{
    ...
    Browser::getInstance().invokeOnUiThread(
        ...
        context, [](void* data) { delete static_cast<FrameContext*>(data); });
}
