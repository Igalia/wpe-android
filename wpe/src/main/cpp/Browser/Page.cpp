...
inline Page* getPage(JNIEnv* env, jobject obj)
{
    return reinterpret_cast<Page*>(env->GetLongField(obj, fields.nativePtr));
}
...
void pageInit(JNIEnv* env, jobject obj, jint pageId, jint width, jint height)
{
    ...
    jweak pageJavaRef = env->NewWeakGlobalRef(obj);
    auto page = new Page(pageJavaRef, width, height, std::make_shared<PageEventObserver>(env, clazz, obj));
    env->SetLongField(obj, fields.nativePtr, reinterpret_cast<intptr_t>(page));
    page->init();
...
void pageDestroy(JNIEnv* env, jobject obj)
{
    ...
    env->SetLongField(obj, fields.nativePtr, 0);
    delete page;
}
...
void pageSurfaceCreated(JNIEnv* env, jobject obj, jobject jSurface)
{
    ...
    ALOGV("pageNativePtr: %ld", env->GetLongField(obj, fields.nativePtr));
...
void pageSurfaceDestroyed(JNIEnv* env, jobject obj)
{
    ...
    auto* page = reinterpret_cast<Page*>(env->GetLongField(obj, fields.nativePtr));
...
Page::Page(jweak javaRef, int width, int height, std::shared_ptr<PageEventObserver> observer)
    : m_javaRef(javaRef)
    , m_width(width)
    , m_height(height)
    , m_observer(observer)
...
Page::~Page()
{
    ...
    if (env) {
        env->DeleteWeakGlobalRef(m_javaRef);
        m_javaRef = nullptr;
    }
}
...
int Page::registerJNINativeFunctions(JNIEnv* env)
{
    ...
    fields.nativePtr = env->GetFieldID(clazz, "nativePtr", "J");
