...
private:
    static void onAutomationStarted(WebKitWebContext* context, WebKitAutomationSession* session, jobject javaRef)
    {
    ...
    static void nativeDestroy(JNIEnv* env, jobject jniObject, jlong contextPtr) noexcept
    {
        UNUSED_PARAM(env);
    ...
    static jboolean nativeIsAutomationMode(JNIEnv* env, jobject jniObject, jlong contextPtr) noexcept
    {
        UNUSED_PARAM(env);
        UNUSED_PARAM(jniObject);
        auto* context = reinterpret_cast<WebKitWebContext*>(contextPtr);
