...
static bool toWpeTouchEventType(jint type, WPEEventType& eventType)
{
    switch (type) {
    ...
    case WPE_EVENT_TOUCH_CANCEL:
        eventType = static_cast<WPEEventType>(type);
        return true;
    default:
        return false;
    }
...
private:
    static void nativeSetToplevel(JNIEnv* env, jobject jniObject, jlong viewPtr, jlong toplevelPtr) noexcept
    {
        UNUSED_PARAM(env);
    ...
    static void nativeResized(JNIEnv* env, jobject jniObject, jlong viewPtr, jint width, jint height) noexcept
    {
        UNUSED_PARAM(env);
        UNUSED_PARAM(jniObject);
        auto* view = reinterpret_cast<WPEView*>(viewPtr);
    ...
    static void nativeSetMapped(JNIEnv* env, jobject jniObject, jlong viewPtr, jboolean mapped) noexcept
    {
        UNUSED_PARAM(env);
        UNUSED_PARAM(jniObject);
        auto* view = reinterpret_cast<WPEView*>(viewPtr);
    ...
    static void nativeDispatchTouchEvent(JNIEnv* env, jobject jniObject, jlong viewPtr, jlong time, jint type,
        jint pointerCount, jintArray ids, jfloatArray xs, jfloatArray ys) noexcept
    {
        ...
        if (!toWpeTouchEventType(type, eventType))
            return;
    ...
    static void nativeDispatchKeyEvent(JNIEnv* env, jobject jniObject, jlong viewPtr, jlong time, jint type,
        jint keyCode, jint unicodeChar, jint metaState) noexcept
    {
        UNUSED_PARAM(env);
        UNUSED_PARAM(jniObject);
        auto* view = reinterpret_cast<WPEView*>(viewPtr);
