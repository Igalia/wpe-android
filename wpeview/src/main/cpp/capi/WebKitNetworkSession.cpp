...
private:
    static jlong nativeInit(JNIEnv*, jobject, jlong contextPtr, jboolean automationMode, jboolean ephemeral,
        jstring dataDir, jstring cacheDir) noexcept
    {
        WebKitNetworkSession* session = nullptr;
        if (automationMode)
            session = g_object_ref(
                webkit_web_context_get_network_session_for_automation(JNI::from_jlong<WebKitWebContext>(contextPtr)));
        else if (ephemeral)
    ...
    static void nativeDestroy(JNIEnv*, jobject, jlong sessionPtr) noexcept
    {
        g_object_unref(JNI::from_jlong<WebKitNetworkSession>(sessionPtr));
    }

    static jlong nativeWebsiteDataManager(JNIEnv*, jobject, jlong sessionPtr) noexcept
    {
        return reinterpret_cast<jlong>(
            webkit_network_session_get_website_data_manager(JNI::from_jlong<WebKitNetworkSession>(sessionPtr)));
    }

    static void nativeSetTLSErrorsPolicy(JNIEnv*, jobject, jlong sessionPtr, jint policy) noexcept
    {
