...
inline float safeDeviceDensity(float density) noexcept
{
    return (density > 0.0F) ? density : 1.0F;
}

void onInputMethodFocusIn(void* userData)
{
    static_cast<WKWebView*>(userData)->onInputMethodContextIn();
}

void onInputMethodFocusOut(void* userData)
{
    static_cast<WKWebView*>(userData)->onInputMethodContextOut();
}
    ...
    , m_deviceScale(deviceScale)
{
    ...
    if (m_wpeView && m_renderer) {
        auto* toplevel = wpe_view_get_toplevel(WPE_VIEW(m_wpeView));
        if (toplevel != nullptr) {
            wpe_toplevel_android_set_renderer(WPE_TOPLEVEL_ANDROID(toplevel), m_renderer);
            wpe_toplevel_android_set_visible_view(WPE_TOPLEVEL_ANDROID(toplevel), m_wpeView);
        }
    }
