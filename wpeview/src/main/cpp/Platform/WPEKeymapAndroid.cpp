...
static gboolean wpeKeymapAndroidGetEntriesForKeyval(WPEKeymap*, guint, WPEKeymapEntry** entries, guint* nEntries)
{
    if (entries)
        *entries = nullptr;
    if (nEntries)
...
static gboolean wpeKeymapAndroidTranslateKeyboardState(
    WPEKeymap*, guint, WPEModifiers, int, guint* keyval, int* effectiveGroup, int* level, WPEModifiers* consumed)
{
    if (keyval)
        *keyval = 0;
    if (effectiveGroup)
    ...
    if (consumed)
        ...
        *consumed = static_cast<WPEModifiers>(0);
    return FALSE;
}

static WPEModifiers wpeKeymapAndroidGetModifiers(WPEKeymap*)
{
    // NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange)
    return static_cast<WPEModifiers>(0);
}
...
static void wpe_keymap_android_init(WPEKeymapAndroid*)
{
}
